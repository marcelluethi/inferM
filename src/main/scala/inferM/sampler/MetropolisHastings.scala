...
class MetropolisHastings[A](initialSample : A) extends DistInterpreter[A, Iterator[A]]: 
  ...
  override def conditional(logLikelihood: A => LogProb, dist: Dist[A]): Iterator[A] = 
  ...
  def mh(dist : Dist[A]): Iterator[A] = 
    ...
    def oneStep(currentSample : (A, LogProb), newProposal : (A, LogProb)) : (A, LogProb) = 
        ...
        if (a > 0.0 || r < a.toProb) then
          newProposal 
        else 
          currentSample
        
    Iterator.iterate((initialSample, LogProb(0.0)))(currentSample => 
