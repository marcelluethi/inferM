...
trait Dist[A]:
  ...
  def condition(logLikelihood: A => LogProb): Dist[A] =
    Conditional(logLikelihood, this)
...
case class Conditional[A](logLikelihood: A => LogProb, dist: Dist[A])
    extends Dist[A]:
  def run[X](interpreter: DistInterpreter[A, X]): X =
    interpreter.conditional(logLikelihood, dist)
